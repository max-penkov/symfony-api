 ...
 */
final class Version20200422092145 extends AbstractMigration
{
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE INDEX IDX_9474526CF675F31B ON comment (author_id)');
        $this->addSql('CREATE INDEX IDX_9474526CA77FBEAF ON comment (blog_post_id)');
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, username, password, name, roles, password_change_date, email, enabled, confirmation_token FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username VARCHAR(255) NOT NULL, password VARCHAR(255) NOT NULL, name VARCHAR(255) NOT NULL, roles CLOB NOT NULL --(DC2Type:simple_array)
        , password_change_date INTEGER DEFAULT NULL, email VARCHAR(255) NOT NULL, confirmation_token VARCHAR(40) DEFAULT NULL, enabled BOOLEAN DEFAULT \'false\' NOT NULL)');
        $this->addSql('INSERT INTO user (id, username, password, name, roles, password_change_date, email, enabled, confirmation_token) SELECT id, username, password, name, roles, password_change_date, email, enabled, confirmation_token FROM __temp__user');
        $this->addSql('DROP TABLE __temp__user');
    }
}
